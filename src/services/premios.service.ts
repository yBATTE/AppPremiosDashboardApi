...
export class PremiosService {
  ...
  private parseAgrLoginForm(html: string) {
    ...
    let form = $("form")
      .filter((_i: number, f: any) => {
        return (
          $(f).find('input[type="password"]').length > 0 ||
          $(f).find('input[name*="password" i]').length > 0 ||
          $(f).find('input[id*="password" i]').length > 0
        );
      })
      .first();
    ...
    form.find("input").each((_i: number, el: any) => {
      ...
      if (
        ...
        (
          ...
          haystack.includes("clave") ||
          haystack.includes("contraseña") ||
          haystack.includes("contrasena")
        )
      ) {
        passField = key;
      }
